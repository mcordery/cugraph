...
def store_results_json(
    benchmark_dir=None, algo_name=None, algo_time=None, n_gpus=None, scale=None
):
    """
    ...
    with open(
        f"{benchmark_dir_path}/benchmark_result_scale_{scale}_ngpus_{n_gpus}_{algo_name}.json",
        "w",
    ) as outfile:
...
def run(
    ...
    try:
        if csv_graph_file:
            ...
            df = funcs.generate_edgelist(
                scale, edgefactor=edgefactor, seed=seed, unweighted=unweighted
            )
            log("done.")
        else:
