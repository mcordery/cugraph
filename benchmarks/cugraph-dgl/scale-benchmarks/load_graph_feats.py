...
def load_edges_from_disk(parquet_path, replication_factor, input_meta):
    ...
    for edge_type in input_meta["num_edges"].keys():
        print(
            f"Loading edge index for edge type {edge_type} for replication factor = {replication_factor}"
        )

        canonical_edge_type = tuple(edge_type.split("__"))
        edge_index = pd.read_parquet(
        ...
        if replication_factor > 1:
            src_list, dst_list = replicate_edges(
                edge_index, canonical_edge_type, replication_factor, input_meta
            )
...
def replicate_edges(edge_index, canonical_edge_type, replication_factor, input_meta):
    ...
    for r in range(1, replication_factor):
        new_src = edge_index["src"] + (
            r * input_meta["num_nodes"][canonical_edge_type[0]]
        )
        new_dst = edge_index["dst"] + (
            r * input_meta["num_nodes"][canonical_edge_type[2]]
        )
...
def load_node_labels(dataset_path, replication_factor, input_meta):
    ...
    for node_type in input_meta["num_nodes"].keys():
        ...
        if os.path.exists(label_path):
            node_data[node_type] = process_node_label(
                label_path, node_type, replication_factor, num_nodes_dict, input_meta
            )
...
def process_node_label(
    label_path, node_type, replication_factor, num_nodes_dict, input_meta
):
    node_label = pd.read_parquet(label_path)

    if replication_factor > 1:
        node_label = replicate_node_label(
            node_label, node_type, replication_factor, input_meta
        )

    node_label_tensor = torch.full(
        (num_nodes_dict[node_type],), -1, dtype=torch.float32
    )
    node_label_tensor[torch.as_tensor(node_label.node.values)] = torch.as_tensor(
        node_label.label.values
    )
    ...
    replicated_df = pd.DataFrame(
        {
            ...
            "label": pd.concat(
                [node_label.label for _ in range(1, replication_factor)]
            ),
        }
    )
