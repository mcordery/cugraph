...
# if(NOT (DEFINED CMAKE_HIP_ARCHITECTURES)) set(CMAKE_HIP_ARCHITECTURES "gfx940;gfx941;gfx942") endif()
...
list(APPEND ROCGRAPH_CXX_FLAGS --offload-arch=gfx942) # need to specify this because I'm building on a machine without
                                                      # an amd card and libhipcxx needs to know for the timers
list(APPEND ROCGRAPH_CXX_FLAGS -DFMT_HEADER_ONLY -DUSE_LIBHIPCXX_PRT)

# list(APPEND ROCGRAPH_CXX_FLAGS "-stdlib=libc++") # need to specify this because I'm building on a machine without an
# amd card and libhipcxx needs to know for the timers
...
# Option to enable line info in HIP device compilation to allow introspection when profiling / memchecking
...
if(BUILD_TESTS)
    # include(${rapids-cmake-dir}/cpm/gtest.cmake) rapids_cpm_gtest(BUILD_STATIC)
endif()

# ######################################################################################################################
# * librocgraph library target --------------------------------------------------

# NOTE: The most expensive compilations are listed first since ninja will run them in parallel in this order, which
...
set(ROCGRAPH_SOURCES
    ...
    # src/community/detail/common_methods_mg.cpp src/community/detail/common_methods_sg.cpp
    # src/community/detail/refine_sg.cpp src/community/detail/refine_mg.cpp
    ...
    # src/sampling/renumber_sampled_edgelist_sg.cpp src/sampling/sampling_post_processing_sg.cpp
    # src/cores/core_number_sg.cpp src/cores/core_number_mg.cpp
    ...
    # src/structure/renumber_edgelist_sg.cpp src/structure/renumber_edgelist_mg.cpp src/structure/renumber_utils_sg.cpp
    # src/structure/renumber_utils_mg.cpp src/structure/relabel_sg.cpp src/structure/relabel_mg.cpp
    # src/structure/induced_subgraph_sg.cpp src/structure/induced_subgraph_mg.cpp
    ...
    # src/traversal/bfs_sg.cpp src/traversal/bfs_mg.cpp src/traversal/sssp_sg.cpp
    # src/traversal/od_shortest_distances_sg.cpp src/traversal/sssp_mg.cpp
    src/link_analysis/hits_sg.cpp
    src/link_analysis/hits_mg.cpp
    # src/link_analysis/pagerank_sg.cpp src/link_analysis/pagerank_mg.cpp
    src/centrality/katz_centrality_sg.cpp
    src/centrality/katz_centrality_mg.cpp
    # src/centrality/eigenvector_centrality_sg.cpp src/centrality/eigenvector_centrality_mg.cpp
    # src/centrality/betweenness_centrality_sg.cpp src/centrality/betweenness_centrality_mg.cpp
    src/tree/legacy/mst.cpp
    # src/components/weakly_connected_components_sg.cpp src/components/weakly_connected_components_mg.cpp
    ...
    # src/community/triangle_count_sg.cpp src/community/triangle_count_mg.cpp src/traversal/k_hop_nbrs_sg.cpp
...
# Per-thread default stream option see https://docs.nvidia.com/cuda/cuda-runtime-api/stream-sync-behavior.html The
# per-thread default stream does not synchronize with other streams
target_compile_definitions(rocgraph PUBLIC HIP_API_PER_THREAD_DEFAULT_STREAM)

# file(WRITE "${ROCGRAPH_BINARY_DIR}/fatbin.ld" [=[ SECTIONS { .nvFatBinSegment : { *(.nvFatBinSegment) } .nv_fatbin : {
# *(.nv_fatbin) } } ]=]) target_link_options(rocgraph PRIVATE "${ROCGRAPH_BINARY_DIR}/fatbin.ld")
...
target_include_directories(
    rocgraph
    PRIVATE "${CMAKE_CURRENT_SOURCE_DIR}/../thirdparty" "${CMAKE_CURRENT_SOURCE_DIR}/src"
    PUBLIC "$<BUILD_INTERFACE:${CMAKE_CURRENT_SOURCE_DIR}/include>" "$<INSTALL_INTERFACE:include>")

# ######################################################################################################################

target_link_libraries(
    rocgraph PUBLIC # hip::host hip::device
                    hipblas hipsparse hiprand hipsolver # libhipcxx::libhipcxx
)
...
# if(BUILD_TESTS) include(CTest) add_subdirectory(tests) endif()
...
install(TARGETS rocgraph DESTINATION ${lib_dir})

install(DIRECTORY include/rocgraph/ DESTINATION include/rocgraph)

install(FILES ${CMAKE_CURRENT_BINARY_DIR}/include/rocgraph/version_config.hpp DESTINATION include/rocgraph)

install(TARGETS rocgraph_c DESTINATION ${lib_dir})

install(DIRECTORY include/rocgraph_c/ DESTINATION include/rocgraph_c)

install(FILES ${CMAKE_CURRENT_BINARY_DIR}/include/rocgraph_c/version_config.hpp DESTINATION include/rocgraph_c)
...
# rapids_export(INSTALL rocgraph EXPORT_SET rocgraph-exports GLOBAL_TARGETS rocgraph rocgraph_c NAMESPACE rocgraph::
# DOCUMENTATION doc_string )

# ######################################################################################################################
# * build export --------------------------------------------------------------- rapids_export(BUILD rocgraph EXPORT_SET
#   rocgraph-exports GLOBAL_TARGETS rocgraph rocgraph_c NAMESPACE rocgraph:: DOCUMENTATION doc_string )

# ######################################################################################################################
# * make documentation --------------------------------------------------------- requires doxygen and graphviz to be
#   installed from build directory, run make docs_rocgraph

# doc targets for rocgraph
find_package(Doxygen 1.8.11)
if(Doxygen_FOUND)
    add_custom_command(
        OUTPUT ROCGRAPH_DOXYGEN
