...
set(CUGRAPH_CXX_FLAGS
    ...
    -Dcugraph_EXPORTS)
# set(CUGRAPH_CUDA_FLAGS -DCUTLASS_NAMESPACE=raft_cutlass -DLIBCUDACXX_ENABLE_EXPERIMENTAL_MEMORY_RESOURCE
# -DRAFT_SYSTEM_LITTLE_ENDIAN=1 -DSPDLOG_FMT_EXTERNAL -DTHRUST_DEVICE_SYSTEM=THRUST_DEVICE_SYSTEM_CUDA
# -DTHRUST_DISABLE_ABI_NAMESPACE -DTHRUST_HOST_SYSTEM=THRUST_HOST_SYSTEM_CPP -DTHRUST_IGNORE_ABI_NAMESPACE_ERROR
# -Dcugraph_EXPORTS)
...
list(APPEND CUGRAPH_CUDA_FLAGS -Werror=cross-execution-space-call -Wno-deprecated-declarations
...
# commented out because not using cugraphops right now if(USE_CUGRAPH_OPS) list(APPEND CUGRAPH_SOURCES
# src/sampling/neighborhood.cu ) endif()
...
if(USE_CUDA)
    ...
    target_compile_options(cugraph PRIVATE "$<$<COMPILE_LANGUAGE:CXX>:${CUGRAPH_CXX_FLAGS}>"
...
file(
    WRITE "${CUGRAPH_BINARY_DIR}/fatbin.ld"
    [=[
SECTIONS
{
...
target_include_directories(
    ...
    PUBLIC "${CUDAToolkit_INCLUDE_DIRS}" "$<BUILD_INTERFACE:${CMAKE_CURRENT_SOURCE_DIR}/include>"
...
if(USE_CUDA)
    target_link_libraries(
        cugraph PUBLIC CUDA::cublas CUDA::cusparse CUDA::curand CUDA::cusolver $<BUILD_LOCAL_INTERFACE:CUDA::toolkit>
        # $<TARGET_NAME_IF_EXISTS:cugraph-ops::cugraph-ops++> PRIVATE ${COMPILED_RAFT_LIB} hipco::hipco
    )
endif()
...
target_compile_options(cugraph_c PRIVATE "$<$<COMPILE_LANGUAGE:CXX>:${CUGRAPH_CXX_FLAGS}>"
                                         "$<$<COMPILE_LANGUAGE:CUDA>:${CUGRAPH_CUDA_FLAGS}>")
...
install(TARGETS cugraph DESTINATION ${lib_dir})

install(DIRECTORY include/cugraph/ DESTINATION include/cugraph)

install(FILES ${CMAKE_CURRENT_BINARY_DIR}/include/cugraph/version_config.hpp DESTINATION include/cugraph)

install(TARGETS cugraph_c DESTINATION ${lib_dir})

install(DIRECTORY include/cugraph_c/ DESTINATION include/cugraph_c)

install(FILES ${CMAKE_CURRENT_BINARY_DIR}/include/cugraph_c/version_config.hpp DESTINATION include/cugraph_c)
...
# rapids_export(INSTALL cugraph EXPORT_SET cugraph-exports GLOBAL_TARGETS cugraph cugraph_c NAMESPACE cugraph::
# DOCUMENTATION doc_string )

# ######################################################################################################################
# * build export --------------------------------------------------------------- rapids_export(BUILD cugraph EXPORT_SET
#   cugraph-exports GLOBAL_TARGETS cugraph cugraph_c NAMESPACE cugraph:: DOCUMENTATION doc_string )

# ######################################################################################################################
# * make documentation --------------------------------------------------------- requires doxygen and graphviz to be
#   installed from build directory, run make docs_cugraph

# doc targets for cugraph
find_package(Doxygen 1.8.11)
if(Doxygen_FOUND)
    add_custom_command(
        OUTPUT CUGRAPH_DOXYGEN
