...
struct __has_iterator_category {
 private:
  ...
  template <class _Up>
  _LIBCUDACXX_EXECUTION_SPACE_SPECIFIER _LIBCUDACXX_INLINE_VISIBILITY static char __test(
    typename _Up::iterator_category* = 0);
...
using __iter_key_type =
  remove_const_t<typename iterator_traits<_InputIterator>::value_type::first_type>;
...
using __iter_to_alloc_type =
  pair<add_const_t<typename iterator_traits<_InputIterator>::value_type::first_type>,
