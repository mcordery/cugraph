 ...
 * 2) The program needs to be compiled with -std=c++20.
 * 3) The linker flag -fgpu-rdc must be set when using multiple translation units.
 * 4) The macro LIBCUDACXX_HIP_DEFINE_SYSCLOCK_VARS needs to be called at file scope level in a single translation unit, usually where the main function is located.
 *    The header "cuda/std/chrono" must be included to make this macro available.
 * 5) hip::std::chrono::hip_gpu_ext::initialize_amdgpu_sysclock_on_current_device() or hip::std::chrono::hip_gpu_ext::initialize_amdgpu_sysclock_on_device()
 *    need to be called on the host once to initialize the system clock for a given device.
 * 6) Subsequent device calls to hip::std::system_clock::now() will then return time_points starting at UNIX time.
 * 
