...
template <class _Type,
          class _Scope,
          typename _CUDA_VSTD::enable_if<sizeof(_Type) <= 2, int>::type = 0>
bool __device__ __atomic_compare_exchange_cuda(_Type volatile* __ptr,
                                               _Type* __expected,
                                               ...
                                               _Scope __s)
{
  ...
  while (1) {
    ...
    if (__old_value != *__expected) break;
    uint32_t const __attempt = (__old & ~__mask) | (*__desired << __offset);
    if (__atomic_compare_exchange_cuda(
          __aligned, &__old, &__attempt, true, __success_memorder, __failure_memorder, __s))
      return true;
  }
...
template <class _Type,
          class _Scope,
          typename _CUDA_VSTD::enable_if<sizeof(_Type) <= 2, int>::type = 0>
void __device__ __atomic_exchange_cuda(
  _Type volatile* __ptr, _Type* __val, _Type* __ret, int __memorder, _Scope __s)
{
  _Type __expected = __atomic_load_n_cuda(__ptr, __ATOMIC_RELAXED, __s);
  while (
    !__atomic_compare_exchange_cuda(__ptr, &__expected, __val, true, __memorder, __memorder, __s))
    ;
  *__ret = __expected;
}
...
__atomic_fetch_add_cuda(_Type volatile* __ptr, _Delta __val, int __memorder, _Scope __s)
{
  ...
  while (!__atomic_compare_exchange_cuda(
    __ptr, &__expected, &__desired, true, __memorder, __memorder, __s))
...
template <
  ...
  class _Delta,
  class _Scope,
  typename _CUDA_VSTD::enable_if<sizeof(_Type) <= 2 || _CUDA_VSTD::is_floating_point<_Type>::value,
                                 int>::type = 0>
_Type __host__ __device__
...
template <class _Type, class _Scope>
bool __device__ __atomic_compare_exchange_n_cuda(_Type volatile* __ptr,
                                                 _Type* __expected,
                                                 _Type __desired,
                                                 bool __weak,
                                                 int __success_memorder,
...
static inline __device__ void __atomic_signal_fence_cuda(int) { asm volatile("" ::: "memory"); }
