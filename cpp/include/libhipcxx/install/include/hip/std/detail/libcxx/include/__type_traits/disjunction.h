...
struct _OrImpl<true> {
  ...
  using _Result _LIBCUDACXX_NODEBUG_TYPE =
    typename _OrImpl<!bool(_First::value) && sizeof...(_Rest) != 0>::template _Result<_First,
                                                                                      _Rest...>;
};
...
using _Or _LIBCUDACXX_NODEBUG_TYPE =
  typename _OrImpl<sizeof...(_Args) != 0>::template _Result<false_type, _Args...>;
...
struct disjunction<_First, _Rest...>
  : _OrImpl<true>::template _Result<false_type, _First, _Rest...> {};
#else
template <class... _Args>
