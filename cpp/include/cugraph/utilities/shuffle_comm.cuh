                     ...
                     rmm::cuda_stream_view stream_view)
{
  ...
  thrust::transform(rmm::exec_policy(stream_view),
                    ...
                    [value_to_group_id_op, group_first, counts = counts.data()] __device__(
                      auto value) -> thrust::tuple<int, size_t> {
                     ...
                     rmm::cuda_stream_view stream_view)
{
  ...
  thrust::transform(rmm::exec_policy(stream_view),
                    ...
                    thrust::make_zip_iterator(
                      thrust::make_tuple(group_ids.begin(), intra_partition_offsets.begin())),
                    [key_to_group_id_op, group_first, counts = counts.data()] __device__(
                      auto key) -> thrust::tuple<int, size_t> {
                                              ...
                                              rmm::cuda_stream_view stream_view)
{
  ...
  auto group_id_first = thrust::make_transform_iterator(
    tx_value_first,
    [value_to_group_id_op] __device__(auto value) -> int { return value_to_group_id_op(value); });
                                              ...
                                              rmm::cuda_stream_view stream_view)
{
  ...
  auto group_id_first = thrust::make_transform_iterator(
    tx_key_first,
    [key_to_group_id_op] __device__(auto key) -> int { return key_to_group_id_op(key); });
  rmm::device_uvector<int> d_tx_dst_ranks(num_groups, stream_view);
  rmm::device_uvector<size_t> d_tx_value_counts(d_tx_dst_ranks.size(), stream_view);
  auto rank_count_pair_first = thrust::make_zip_iterator(
    thrust::make_tuple(d_tx_dst_ranks.begin(), d_tx_value_counts.begin()));
