...
inline cuda_device_id get_current_cuda_device()
{
  ...
  RMM_ASSERT_CUDA_SUCCESS(hipGetDevice(&dev_id));
...
inline int get_num_cuda_devices()
{
  ...
  RMM_ASSERT_CUDA_SUCCESS(hipGetDeviceCount(&num_dev));
  return num_dev;
}
...
struct cuda_set_device_raii {
  ...
  {
    if (needs_reset_) { RMM_ASSERT_CUDA_SUCCESS(hipSetDevice(dev_id.value())); }
  }
  /**
  ...
  {
    if (needs_reset_) { RMM_ASSERT_CUDA_SUCCESS(hipSetDevice(old_device_.value())); }
  }
