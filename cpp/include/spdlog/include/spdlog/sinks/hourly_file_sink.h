...
struct hourly_filename_calculator {
  ...
  static filename_t calc_filename(const filename_t& filename, const tm& now_tm)
  {
    ...
    return fmt_lib::format(SPDLOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}_{:02d}{}"),
                           basename,
                           now_tm.tm_year + 1900,
                           now_tm.tm_mon + 1,
                           now_tm.tm_mday,
...
class hourly_file_sink final : public base_sink<Mutex> {
 public:
      ...
      filenames_q_()
  {
    ...
    if (max_files_ > 0) { init_filenames_q_(); }
  }
 ...
 protected:
  void sink_it_(const details::log_msg& msg) override
  {
    ...
    if (should_rotate && max_files_ > 0) { delete_old_(); }
  }
  ...
  void delete_old_()
  {
    ...
    if (filenames_q_.full()) {
      ...
      if (!ok) {
        ...
        SPDLOG_THROW(
          spdlog_ex("Failed removing hourly file " + filename_to_str(old_filename), errno));
      }
    }
    filenames_q_.push_back(std::move(current_file));
  }
