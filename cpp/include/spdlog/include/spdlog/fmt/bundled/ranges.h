...
class is_std_string_like {
 ...
 public:
  static constexpr const bool value = is_string<T>::value ||
...
struct format_tuple_element {
  ...
  {
    if (i > 0) ctx.advance_to(detail::copy_str<char_type>(separator, ctx.out()));
...
struct formatter<
 ...
 public:
  ...
  {
    auto it = ctx.begin();
    if (it != ctx.end() && *it != '}') FMT_THROW(format_error("invalid format specifier"));
...
struct is_range {
  static constexpr const bool value = detail::is_range_<T>::value &&
                                      !detail::is_std_string_like<T>::value &&
...
using range_formatter_type = formatter<
  remove_cvref_t<decltype(range_mapper<buffer_context<Char>>{}.map(std::declval<Element>()))>,
  Char>;
 ...
 private:
                              ...
                              std::integral_constant<size_t, N>) -> decltype(ctx.begin())
  {
    ...
    if (N > 1) {
      ...
      if (end != end1) FMT_THROW(format_error("incompatible format specs for tuple elements"));
    }
#endif
    return end;
  }
