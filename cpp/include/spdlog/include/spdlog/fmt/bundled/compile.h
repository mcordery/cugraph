...
FMT_CONSTEXPR inline auto copy_str(InputIt begin,
                                   InputIt end,
...
struct runtime_named_field {
  ...
  constexpr OutputIt format(OutputIt out, const Args&... args) const
  {
    ...
    if (!found) { FMT_THROW(format_error("argument with specified name is not found")); }
    return out;
  }
};
                                                  ...
                                                  int next_arg_id)
{
  ...
  return {f,
          pos + fmt::detail::to_unsigned(end - str.data()),
...
constexpr auto compile_format_string(S format_str)
{
  ...
  if constexpr (str[POS] == '{') {
    if constexpr (POS + 1 == str.size()) FMT_THROW(format_error("unmatched '{' in format string"));
    if constexpr (str[POS + 1] == '{') {
      ...
      if constexpr (arg_id_result.arg_id.kind == arg_id_kind::index) {
        ...
        if constexpr (arg_index >= 0) {
          constexpr auto next_id = ID != manual_indexing_id ? ID + 1 : manual_indexing_id;
          return parse_replacement_field_then_tail<decltype(get_type<arg_index, Args>::value),
                                                   Args,
                                                   arg_id_end_pos,
                                                   arg_index,
                                                   next_id>(format_str);
...
template <typename S, typename... Args, FMT_ENABLE_IF(detail::is_compiled_string<S>::value)>
FMT_CONSTEXPR20 auto formatted_size(const S& format_str, const Args&... args) -> size_t
{
