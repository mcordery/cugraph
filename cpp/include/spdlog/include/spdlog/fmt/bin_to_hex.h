...
struct formatter<spdlog::details::dump_info<T>, char> {
    ...
    while (it != ctx.end() && *it != '}') {
      switch (*it) {
        ...
        case 'n':
          ...
          break;
        case 'a':
          if (put_newlines) { show_ascii = true; }
          break;
      }
              ...
              FormatContext& ctx) const -> decltype(ctx.out())
  {
    ...
    for (auto i = the_range.get_begin(); i != the_range.get_end(); i++) {
      ...
      if (put_delimiters && i != the_range.get_begin()) { *inserter++ = delimiter; }
    ...
    if (show_ascii)  // add ascii to last line
    {
      if (the_range.get_end() - the_range.get_begin() > size_per_line) {
        ...
        while (blank_num-- > 0) {
          *inserter++ = delimiter;
          *inserter++ = delimiter;
          if (put_delimiters) { *inserter++ = delimiter; }
        }
      }
      *inserter++ = delimiter;
      *inserter++ = delimiter;
  ...
  void put_newline(It inserter, std::size_t pos) const
  {
    ...
    *inserter++ = '\n';
    ...
    if (put_positions) { spdlog::fmt_lib::format_to(inserter, SPDLOG_FMT_STRING("{:04X}: "), pos); }
  }
};
}  // namespace std
