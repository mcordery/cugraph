...
class MmaTensorOpFragmentIterator<Shape_,
 ...
 public:
  ...
  struct Policy {
    ...
    static_assert(!(AccumulatorShape::kColumn % Shape::kColumn),
 ...
 private:
  ...
  static int const kKBlockColumnIterations =
    kKBlockColumn / Shape::kColumn * (AccumulatorShape::kRow / Shape::kRow);
  static int const kResidualIndex =
    kResidualColumn / Shape::kColumn * (AccumulatorShape::kRow / Shape::kRow);
 ...
 public:
  ...
  using ScaleBiasFragment =
    Array<ElementScaleBias, InstructionShape::kM * InstructionShape::kK / kThreads>;
 ...
 public:
  ...
  MmaTensorOpFragmentIterator(AccumulatorFragment const& accum)
    : accumulators_(reinterpret_cast<AccessType const*>(&accum)), index_(0), is_residual_tile_(true)
  {
  }
  ...
  void load(Fragment& frag, OutputOp output_op) const
  {
    ...
    for (int n = 0; n < MmaIterations::kColumn; n++) {
      for (int m = 0; m < MmaIterations::kRow; m++) {
        int accumulator_access_offset = n * AccumulatorIterations::kRow + m + index;
  ...
  void load(Fragment& frag,
            ScaleBiasFragment& scale,
            ScaleBiasFragment& bias,
      ...
      for (int m = 0; m < MmaIterations::kRow; m++) {
        ...
        if (!(is_residual_tile_ && index_ >= kResidualIndex))
          frag_ptr[m * MmaIterations::kColumn + n] = output_op(
            accumulators_[accumulator_access_offset], scale_ptr[n] /*scale*/, bias_ptr[n] /*bias*/);
      }
    }
  }
};
  ...
  void load(Fragment& frag, OutputOp output_op) const
  {
    ...
    for (int i = 0; i < AccessIterations::kCount; i++) {
      int accumulator_access_offset =
        index / AccessIterations::kCount * (MmaIterations::kColumn * kIterationsPerInstruction) +
        (index % AccessIterations::kCount) /
          (AccessIterations::kColumn * kIterationsPerInstruction) * AccumulatorIterations::kColumn *
          kIterationsPerInstruction +
        (index % (AccessIterations::kColumn * kIterationsPerInstruction)) /
          kIterationsPerInstruction * (kIterationsPerInstruction * kIterationsPerAccess) +
      ...
      for (int j = 0; j < kIterationsPerAccess; j++) {
        ...
        if (!(is_residual_tile_ && index_ >= kResidualIndex))
          frag_ptr[i * kIterationsPerAccess + j] =
            output_op(accumulators_[accumulator_access_offset + j * kAccessStride]);
      }
      index++;
    }
  }
            ...
            OutputOp output_op) const
  {
    ...
    for (int i = 0; i < AccessIterations::kCount; i++) {
      ...
      int scale_bias_offset =
        (index % (kIterationsPerInstruction * AccessIterations::kColumn)) * kIterationsPerAccess;
