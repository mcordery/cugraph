...
struct formatter<std::filesystem::path, Char> {
 ...
 public:
  ...
  auto format(const std::filesystem::path& p, FormatContext& ctx) const
  {
    auto specs = specs_;
    auto path_string =
      !path_type_ ? p.native() : p.generic_string<std::filesystem::path::value_type>();
...
struct formatter<std::optional<T>, Char, std::enable_if_t<is_formattable<T, Char>::value>> {
 private:
  ...
  template <class U>
  FMT_CONSTEXPR static auto maybe_set_debug_format(U& u,
                                                   bool set) -> decltype(u.set_debug_format(set))
  {
...
class is_variant_formattable_ {
  ...
  static std::conjunction<is_formattable<std::variant_alternative_t<Is, T>, C>...> check(
...
struct formatter<Variant,
  ...
  auto format(const Variant& value, FormatContext& ctx) const -> decltype(ctx.out())
  {
    ...
    FMT_TRY
    {
      std::visit([&](const auto& v) { out = detail::write_escaped_alternative<Char>(out, v); },
                 value);
    }
...
struct formatter<T,
 ...
 public:
  ...
  {
    auto out = ctx.out();
    if (!with_typename_) return detail::write_bytes<Char>(out, string_view(ex.what()));
...
struct formatter<std::complex<F>, Char> : nested_formatter<F, Char> {
 private:
  ...
  struct writer {
    ...
    {
      if (c.real() != 0) {
        auto format_full = detail::string_literal<Char, '(', '{', '}', '+', '{', '}', 'i', ')'>{};
        return fmt::format_to(
          out, basic_string_view<Char>(format_full), f->nested(c.real()), f->nested(c.imag()));
      }
      auto format_imag = detail::string_literal<Char, '{', '}', 'i'>{};
      return fmt::format_to(out, basic_string_view<Char>(format_imag), f->nested(c.imag()));
    }
  };

 public:
  template <typename FormatContext>
  auto format(const std::complex<F>& c, FormatContext& ctx) const -> decltype(ctx.out())
  {
