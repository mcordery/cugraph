...
inline auto write_loc(basic_appender<wchar_t> out,
                      loc_value value,
...
constexpr auto operator""_a(const wchar_t* s, size_t) -> detail::udl_arg<wchar_t> { return {s}; }
#endif
}  // namespace literals
...
template <typename Locale,
          ...
          typename Char = detail::format_string_char_t<S>,
          FMT_ENABLE_IF(detail::is_locale<Locale>::value&& detail::is_exotic_char<Char>::value)>
inline auto format(const Locale& loc, const S& format_str, T&&... args) -> std::basic_string<Char>
{
  return detail::vformat(loc,
...
{
  return vformat_to(
    out, detail::to_string_view(fmt), fmt::make_format_args<buffered_context<Char>>(args...));
}
...
template <typename OutputIt,
          ...
          bool enable   = detail::is_output_iterator<OutputIt, Char>::value &&
                        detail::is_locale<Locale>::value && detail::is_exotic_char<Char>::value>
inline auto format_to(OutputIt out, const Locale& loc, const S& format_str, T&&... args) ->
...
template <typename OutputIt,
          ...
          FMT_ENABLE_IF(detail::is_output_iterator<OutputIt, Char>::value&&
                          detail::is_exotic_char<Char>::value)>
inline auto vformat_to_n(OutputIt out,
                         size_t n,
...
inline auto formatted_size(const S& fmt, T&&... args) -> size_t
{
  ...
  detail::vformat_to(
    buf, detail::to_string_view(fmt), fmt::make_format_args<buffered_context<Char>>(args...));
  return buf.count();
}
...
inline auto vformat(const text_style& ts, wstring_view fmt, wformat_args args) -> std::wstring
{
  auto buf = wmemory_buffer();
...
template <typename... T>
inline auto format(const text_style& ts, wformat_string<T...> fmt, T&&... args) -> std::wstring
{
  return fmt::vformat(ts, fmt, fmt::make_wformat_args(args...));
}

template <typename... T>
FMT_DEPRECATED void print(std::FILE* f,
                          const text_style& ts,
