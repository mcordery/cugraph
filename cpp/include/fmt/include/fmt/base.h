...
struct is_std_string_like<
  T,
  void_t<decltype(std::declval<T>().find_first_of(typename T::value_type(), 0))>> : std::true_type {
};
...
constexpr auto is_utf8_enabled() -> bool
{
  ...
  return sizeof("\u00A7") == 3 && uchar("\u00A7"[0]) == 0xC2 && uchar("\u00A7"[1]) == 0xA7;
}
constexpr auto use_utf8() -> bool { return !FMT_MSC_VERSION || is_utf8_enabled(); }
...
struct is_back_insert_iterator<
  It,
  bool_constant<
    std::is_same<decltype(back_inserter(std::declval<typename It::container_type&>())), It>::value>>
  : std::true_type {};
...
class buffer {
 ...
 protected:
  ...
  FMT_CONSTEXPR20 buffer(grow_fun grow, size_t sz) noexcept : size_(sz), capacity_(sz), grow_(grow)
  {
  }
...
class iterator_buffer<T*, T, fixed_buffer_traits> : public fixed_buffer_traits, public buffer<T> {
 private:
  ...
  static FMT_CONSTEXPR void grow(buffer<T>& buf, size_t)
  {
    if (buf.size() == buf.capacity()) static_cast<iterator_buffer&>(buf).flush();
  }
 ...
 public:
  ...
  iterator_buffer(iterator_buffer&& other) noexcept
    : fixed_buffer_traits(other), buffer<T>(static_cast<iterator_buffer&&>(other)), out_(other.out_)
  {
    if (this->data() != out_) {
      this->set(data_, buffer_size);
      this->clear();
...
template <typename OutputIt>
class iterator_buffer<OutputIt,
 ...
 public:
  explicit iterator_buffer(container_type& c) : buffer<value_type>(grow, c.size()), container_(c) {}
  explicit iterator_buffer(OutputIt out, size_t = 0) : iterator_buffer(get_container(out)) {}
...
FMT_CONSTEXPR void basic_format_parse_context<Char>::do_check_arg_id(int id)
{
  ...
  if (detail::is_constant_evaluated() && (!FMT_GCC_VERSION || FMT_GCC_VERSION >= 1200)) {
    ...
    if (id >= static_cast<context*>(this)->num_args()) report_error("argument not found");
  }
}
...
class basic_appender {
 private:
  ...
  friend auto get_container(basic_appender app) -> detail::buffer<T>& { return *app.buffer_; }
...
template <typename T>
auto get_buffer(basic_appender<T> out) -> buffer<T>&
{
...
class value {
 public:
  ...
  constexpr FMT_ALWAYS_INLINE value(unsigned long long val) : ulong_long_value(val) {}
...
struct arg_mapper {
  ...
  FMT_MAP_API auto map(unsigned long long val) -> unsigned long long { return val; }
...
struct is_output_iterator<It, T, void_t<decltype(*std::declval<It&>()++ = std::declval<T>())>>
  : std::true_type {};
...
struct format_arg_store {
  ...
  FMT_MAP_API format_arg_store(T&... values)
    : args{{named_args, NUM_NAMED_ARGS}, make_arg<NUM_ARGS <= max_packed_args, Context>(values)...}
  {
    using dummy   = int[];
    int arg_index = 0, named_arg_index = 0;
    (void)dummy{0, (init_named_arg(named_args, arg_index, named_arg_index, values), 0)...};
  }
  ...
  */
  ...
  FMT_CONSTEXPR auto visit(Visitor&& vis) -> decltype(vis(0))
  {
    switch (type_) {
      ...
      case detail::type::int128_type: return vis(detail::convert_for_visit(value_.int128_value));
      case detail::type::uint128_type: return vis(detail::convert_for_visit(value_.uint128_value));
...
class basic_format_args {
 ...
 public:
  ...
  constexpr basic_format_args(const format_arg* args, int count)
    : desc_(detail::is_unpacked_bit | detail::to_unsigned(count)), args_(args)
  {
  }
...
class context {
 ...
 public:
  ...
  FMT_CONSTEXPR auto arg_id(string_view name) -> int { return args_.get_id(name); }
...
using basic_format_context =
  conditional_t<std::is_same<OutputIt, appender>::value, context, generic_context<OutputIt, Char>>;
...
struct fill_t {
 ...
 public:
  ...
  {
    using uchar = unsigned char;
    return static_cast<Char>(static_cast<uchar>(data_[0]) | (static_cast<uchar>(data_[1]) << 8));
  }
...
template <typename Char, typename Handler>
FMT_CONSTEXPR auto parse_arg_id(const Char* begin,
                                const Char* end,
                                Handler&& handler) -> const Char*
{
                                      ...
                                      type arg_type) -> const Char*
{
  ...
  struct {
    ...
    {
      if (current_state >= s || !valid) report_error("invalid format specifier");
      current_state = s;
    }
  } enter_state;
...
FMT_INLINE void print(format_string<T...> fmt, T&&... args)
{
  ...
  return detail::is_locking<T...>() ? vprint_buffered(stdout, fmt, vargs) : vprint(fmt, vargs);
}
...
FMT_INLINE void print(FILE* f, format_string<T...> fmt, T&&... args)
{
  ...
  return detail::is_locking<T...>() ? vprint_buffered(f, fmt, vargs) : vprint(f, fmt, vargs);
}
