...
struct has_member_fn_begin_end_t<
  T,
  void_t<decltype(std::declval<T>().begin()), decltype(std::declval<T>().end())>> : std::true_type {
};
...
struct has_const_begin_end<
  T,
  void_t<decltype(detail::range_begin(std::declval<const remove_cvref_t<T>&>())),
         decltype(detail::range_end(std::declval<const remove_cvref_t<T>&>()))>> : std::true_type {
};

template <typename T>
struct has_mutable_begin_end<T,
...
struct is_range_<T, void>
  : std::integral_constant<bool,
                           (has_const_begin_end<T>::value || has_mutable_begin_end<T>::value)> {};
  ...
  static auto check(index_sequence<Is...>)
    -> decltype(all_true(
      ...
      integer_sequence<bool,
                       (is_formattable<typename std::tuple_element<Is, T>::type, C>::value)...>{}));
...
{
  for_each(
    tuple_index_sequence<remove_cvref_t<Tuple>>(), std::forward<Tuple>(t), std::forward<F>(f));
}
...
FMT_CONSTEXPR auto maybe_set_debug_format(Formatter& f,
...
struct formatter<
  Tuple,
  Char,
  enable_if_t<fmt::is_tuple_like<Tuple>::value && fmt::is_tuple_formattable<Tuple, Char>::value>> {
 private:
 ...
 public:
  ...
  auto format(const Tuple& value, FormatContext& ctx) const -> decltype(ctx.out())
  {
    ...
    detail::for_each2(
      formatters_, value, detail::format_tuple_element<FormatContext>{0, ctx, separator_});
...
using range_formatter_type = formatter<
  remove_cvref_t<decltype(range_mapper<buffered_context<Char>>{}.map(std::declval<Element>()))>,
  Char>;
...
struct range_formatter<
  T,
  Char,
  enable_if_t<conjunction<std::is_same<T, remove_cvref_t<T>>, is_formattable<T, Char>>::value>> {
 private:
  ...
  FMT_CONSTEXPR auto parse(ParseContext& ctx) -> decltype(ctx.begin())
  {
    ...
    switch (detail::to_ascii(*it)) {
      ...
      case '?':
        ...
        FMT_FALLTHROUGH;
      case 's':
        if (!std::is_same<T, Char>::value) report_error("invalid format specifier");
        if (!is_debug) {
...
struct range_format_kind
  : conditional_t<is_range<T, Char>::value,
...
// A map formatter.
template <typename R, typename Char>
struct formatter<R, Char, enable_if_t<range_format_kind<R, Char>::value == range_format::map>> {
 private:
...
struct join_view : detail::view {
  ...
  join_view(It b, Sentinel e, basic_string_view<Char> s) : begin(std::move(b)), end(e), sep(s) {}
};
...
struct formatter<join_view<It, Sentinel, Char>, Char> {
 ...
 public:
  ...
  template <typename FormatContext>
  auto format(view_ref& value, FormatContext& ctx) const -> decltype(ctx.out())
  {
...
struct formatter<tuple_join_view<Char, T...>, Char> {
  ...
  template <typename FormatContext>
  auto format(const tuple_join_view<Char, T...>& value, FormatContext& ctx) const ->
 ...
 private:
  ...
  FMT_CONSTEXPR auto do_parse(ParseContext& ctx,
                              std::integral_constant<size_t, 0>) -> decltype(ctx.begin())
  {
    return ctx.begin();
  }
  ...
  FMT_CONSTEXPR auto do_parse(ParseContext& ctx,
                              std::integral_constant<size_t, N>) -> decltype(ctx.begin())
  {
    auto end = ctx.begin();
    ...
    if (N > 1) {
      ...
      if (end != end1) report_error("incompatible format specs for tuple elements");
    }
#endif
    return end;
  }
  ...
  template <typename FormatContext>
  auto do_format(const tuple_join_view<Char, T...>&,
                 FormatContext& ctx,
                 std::integral_constant<size_t, 0>) const -> typename FormatContext::iterator
  {
    return ctx.out();
  }

  template <typename FormatContext, size_t N>
  auto do_format(const tuple_join_view<Char, T...>& value,
                 FormatContext& ctx,
                 std::integral_constant<size_t, N>) const -> typename FormatContext::iterator
  {
    auto out =
      std::get<sizeof...(T) - N>(formatters_).format(std::get<sizeof...(T) - N>(value.tuple), ctx);
    if (N <= 1) return out;
